...
def ensure_tensor(
    x: typing.Union[np.ndarray, jnp.ndarray, float, int],
    ndim: int = None,
    batch_size: int = None,
):
    ...
    """
    if not is_tensor(x):
        x = jnp.array(x)  # todo: double check
    if ndim is not None:
        assert x.ndim <= ndim
        while x.ndim < ndim:
...
def hz_to_bin(hz: jnp.ndarray, n_fft: int, sample_rate: int):
    ...
    shape = hz.shape
    hz = hz.flatten()
    freqs = jnp.linspace(start=0, stop=sample_rate / 2, num=2 + n_fft // 2)
    hz[hz > sample_rate / 2] = sample_rate / 2

    closest = jnp.abs(hz[None, :] - freqs[:, None])
