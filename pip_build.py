...
"""Script to create (and optionally install) a `.whl` archive for audiotools.
...
def copy_source_to_build_directory(root_path):
    # Copy sources (`audiotools/` directory and setup files) to build
    # directory
    os.chdir(root_path)
...
def run_namex_conversion():
    # Restructure the codebase so that source files live in `audiotools/src`
    namex.convert_codebase(package, code_directory="src")

    # Generate API __init__.py files in `audiotools/`
...
def export_version_string(version, is_nightly=False, rc_index=None):
    ...
    if is_nightly:
        ...
        # Replaces `name="audiotools"` string in `setup.py` with `audiotools-nightly`
        with open("setup.py") as f:
            setup_contents = f.read()
        with open("setup.py", "w") as f:
            setup_contents = setup_contents.replace(
                'name="audiotools"', 'name="audiotools-nightly"'
            )
...
def build(root_path, is_nightly=False, rc_index=None):
    ...
    try:
        ...
        from audiotools.src.version import __version__  # noqa: E402
